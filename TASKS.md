...
Current: _(none yet — start with 1)_

- [ ] 1. Scaffolding & leaf modules (`package.json`, `tsconfig.json`, `biome.json`, install deps, `src/config.ts`, `src/errors.ts`, config unit test). After this `pnpm typecheck && pnpm lint && pnpm test` passes.
