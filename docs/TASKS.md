...
Current: 5 done, all tasks complete.
...
- [x] 4. Connection layer (`src/conn.ts` — per-connection state, rate-limit token bucket, validate→mutate→broadcast handler structure, idempotent `onClose` cleanup).
- [x] 5. Wire-up (`src/index.ts` — Hono app, `/health`, `/ws` upgrade with `room`/`name` validation, heartbeat loop, SIGTERM shutdown, the two `try/catch` error boundaries. No new unit tests — acknowledged integration gap).